...
function App() {
  ...
  const [selectedPrinter, setSelectedPrinter] = useState("fd");
  ...
  useEffect(() => {
    if (window.electron) {
      // Real-time flow: main process emits "jobs-update" whenever Firestore snapshot changes.
      // This callback runs for initial load + every new/updated/removed job in that query.
      window.electron.onJobsUpdate((data) => {
        // Replace the local list with the latest server-authoritative jobs array.
        setJobs(data);
      });
  ...
  const handleConfirmReject = async () => {
    ...
    await window.electron.rejectJob({
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 space-y-4">
            ...
            <p className="text-gray-600 text-sm">
              Are you sure you want to reject this job? It will be moved to
              History.
            </p>
